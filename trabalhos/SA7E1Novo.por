programa
{
	...
	funcao inicio()
	{
		...
		se(maxPolt > 300)
		{
			...
			se(linMax > linha)
				{ 
				escreva("LIMITE DE LINHAS ATINGIDO")
				linMax = 0 
				u.aguarde(1500)
				inicio()
			}
		...
		se((linMax * colMax) > maxPolt)
		{
			escreva("valor não suportado...\n")
			escreva("cadatre novamente!\n")
			linMax = 0
			colMax = 0 
			maxPolt = 0.0
			u.aguarde(1500)
 ...
 * 
 * @POSICAO-CURSOR = 3521; 
 * @DOBRAMENTO-CODIGO = [25, 35, 42, 61, 72, 82, 189, 206, 225, 265];
 * @PONTOS-DE-PARADA = ;
